...
class Scraper
    ...
    def self.retrieve_single_plant_info(url)
        ...
        description_array.pop(6) # Removes ["View propagation protocol from Native Plants Network.", "Go back", "4801 La Crosse Ave.Austin, TX 78739512.232.0100MapContact Us", "\n", "\n" (and something else?)] 
    ...
    def self.scrape_search_page(search_page_url)
        ...
        if !doc.css("p").text.include?("Your search did not return any results, please try again.")
            ...
            @@last_search_page_scraped = partial_urls.map { |url_half| url_half = ('https://www.wildflower.org/plants/' + url_half) }
